...
export async function toggleFavorite(productId: string) {
  // Auth check...
  const [product] = await db.select().from(beautyProducts)
    .where(eq(beautyProducts.id, productId)).limit(1);
  if (!product) throw new Error("Product not found");

  const [updated] = await db.update(beautyProducts)
    .set({ isFavorite: product.isFavorite === 1 ? 0 : 1, updatedAt: new Date() })
